...
def run():
    ...
    try:
        ...
        assert len(bodies) >= 1, f"plain comment was not posted (no body recorded): {bodies}"
        plain = bodies[0]
        assert "comment_text" in plain and "plain hello" in plain, \
            f"plain comment did not use the plain-text path: {plain}"
        assert '"type":"tag"' not in plain, f"a plain comment must carry no mention tag: {plain}"
        assert app.on_detail(), "posting a plain comment should stay on the detail:\n" + app.visible()
        ...
        assert "@" + MEMBER_NAME in app.visible(), \
            "the @Ada Lovelace token was not inserted into the composer:\n" + app.visible()

        app.key(b"\t", settle=1.0)               # Tab → Post button
        app.key(b"\r", settle=2.5)               # Enter → post
        bodies = app.posted_bodies()
        assert len(bodies) >= 2, f"mention comment was not posted (only {len(bodies)} bodies): {bodies}"
        mention = bodies[1]
        assert '"type":"tag"' in mention, f"mention comment carried no structured tag block: {mention}"
        assert f'"id":{MEMBER_ID}' in mention, \
            f"mention tag did not reference member {MEMBER_ID}: {mention}"
        assert '"text":"hi ' in mention, f"mention body lost its leading text run: {mention}"
        assert "comment_text" not in mention, \
            f"a structured mention post must not also send comment_text: {mention}"

        # The posted mention renders in the pane (its visible @Name literal), stacked over the composer
        # which has now closed.
        assert "@" + MEMBER_NAME in app.visible(), \
            "the posted mention comment did not render in the detail pane:\n" + app.visible()
    finally:
        app.kill()


run()
print(f"ok — plain comment → plain-text path (comment_text, no tag); @-mention → structured tag "
      f"block for member {MEMBER_ID} (Ada Lovelace), token rendered in composer and pane")
