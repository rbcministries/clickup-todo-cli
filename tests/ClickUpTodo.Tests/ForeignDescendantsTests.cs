...
public sealed class ForeignDescendantsTests
{
    ...
    // The old interim helper `ForeignSubtasksNotUnderPinned` (which decided "under a pin?" from the first
    // in-snapshot ancestor) was retired in #85; its responsibility now lives in FocusSectionLayout, which
    // pulls a pin's foreign descendants into the Focus section and returns them in NestedSubtaskIds — see
    // FocusSectionLayoutTests. That fixes the transitive case the ancestor-only rule got wrong.
}
