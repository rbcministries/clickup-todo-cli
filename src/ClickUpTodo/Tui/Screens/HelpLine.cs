...
public static class HelpItemSets
{
    ...
    /// When no overlay is open the set depends on whether the Task Tree tab is present (its F6 badge
    /// cycle, #415): present → <see cref="DetailWithTaskTree"/>, absent (e.g. single-task launch mode) →
