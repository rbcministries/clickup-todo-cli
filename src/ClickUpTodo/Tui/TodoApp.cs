...
public sealed class TodoApp
{
    ...
    private void ShowQuickOpenSurface()
    {
        if (NativeModalSpike.Enabled)
        {
            ...
            // with no AddTimeout deferral (the workaround the _screens leg needs below). The launch-chord
            // override (#506) threads through so a rebound chord fires on the native leg too. Flag-gated;
            // off in production.
            if (NativeModalSpike.TryBeginOpenQuickOpen())
                Application.Invoke(() => NativeModalSpike.RunQuickOpenDialog(DispatchQuickOpen, Flash, _launchChords));
            return;
        }
