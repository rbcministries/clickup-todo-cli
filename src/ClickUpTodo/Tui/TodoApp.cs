...
public sealed class TodoApp
{
        ...
        if (key.IsCtrl)
        {
            switch (key.KeyCode & ~KeyCode.CtrlMask)
            {
                ...
                case KeyCode.O:
                    ...
                    break;
                case KeyCode.Enter:
                    // Ctrl+Enter opens the focused task in its own terminal tab (#301) — a single-task
                    // `clickup-todo --task <id>` launch. A chord, so bare Enter stays "open detail
                    // in-process". Ctrl+Left-Click on a row is the mouse equivalent (OnListMouse).
                    key.Handled = true;
                    LaunchTaskInNewTab(CurrentTask());
                    break;
                case KeyCode.U:
    ...
    /// <summary>
    /// Mouse gestures on a task row, resolved via the shared <see cref="RowHitTester"/> (the click's
    /// viewport-relative Y plus the list's scroll offset <c>Viewport.Y</c>): <b>Ctrl+Left-Click</b> opens
    /// the task in its own terminal tab (#301, the mouse equivalent of Ctrl+Enter), and a plain
    /// <b>double-click</b> opens its Task Detail (#286, the mouse equivalent of Enter). Every other mouse
    /// event is left unhandled, so the ListView's native single-click select and drag-scroll are
    /// untouched; a gesture on a header/spacer row or the empty space beneath a short list resolves to a
    /// null task and no-ops, exactly like the keyboard there. Guarded on <see cref="ActiveScreen"/> so a
    /// click can't fire while a screen is stacked over the list.
    /// </summary>
    private void OnListMouse(object? sender, Mouse e)
    {
        ...
        // of Ctrl+Enter. Checked before the plain double-click branch so a Ctrl-modified click launches
        // a tab rather than falling through to open-detail. Resolves the row with the shared hit-tester.
        ...
        // Double-click a task row → open its Task Detail, the mouse equivalent of Enter (#286).
